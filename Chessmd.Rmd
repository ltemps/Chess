...
pacman::p_load(readr, readxl, tidyverse, plyr, dplyr, lmerTest, gvlma, ggplot2, plotly, gridExtra, VGAM, nnet)
```
...
```{r, echo=FALSE, fig.height=4, fig.width= 10,fig.cap="caption of visualization"}
ggplot(Chess, aes(x= WhiteElo, y= Winner)) +
  geom_point(aes(color= Opening), alpha = 0.3) +
  labs(title= "Opening move & Match Winner", 
       x= "White Elo Rating", y= "Winner") +
...
```{r}
#m_3 <- lmer(Number_of_Moves ~ Opening + (1 | WhiteElo), Chess)
m_3 <- lmer(Number_of_Moves ~ Opening + (1 | WhiteElo) + (1 | Opening:WhiteElo), Chess)
...
summary(m_3)
#coef(m_3)
#confint(m_3) #lower and upper bounds
```


Below we see the fixed effects, ____ variables are significant at alpha = 0.05 level, etc.

|                    |Estimate   |Std. Error  |df        |t value |Pr(>&#124;t&#124;) |
|:---:               |:---:      |:---:       |:---:     |:---:   |:---:              |
|(Intercept)         |39.6590    |0.1322      |2975.8553 |299.912 | < 2e-16 ***       |
|English Opening     |0.4492     |0.1782      |2649.7683 |2.521   |0.011763 *         |
|French Defense      |-0.4186    |0.1753      |2581.4494 |-2.387  |0.017049 *         |
|Hungarian Opening   |0.6302     |0.1889      |2866.0539 |3.337   |0.000859 ***       |
|Modern Defense      |-0.4341    |0.1952      |4067.3991 |-2.224  |0.026211 *         |
|Nimzo-Larsen Attack |0.2279     |0.2036      |3192.6362 |1.120   |0.262973           |
|Queen's Pawn Game   |0.3518     |0.2000      |4402.1877 |1.759   |0.078679 .         |
|Scandinavian Defense|-0.7048    |0.1989      |4497.1876 |-3.543  |0.000400 ***       |
|Sicilian Defense    |-0.3649    |0.1543      |1396.8587 |-2.365  |0.018188 *         |
|Zukertort Opening   |0.4473     |0.1920      |4347.5291 |2.330   |0.019839 *         |
...
binnedplot(fitted(m_2),resid(m_2))
re <- plot(m_2)
qq <- qqmath(m_2)
...
ggplot(data.frame(lev=hatvalues(m_2),pearson=residuals(m_2,type="pearson")),
      aes(x=lev,y=pearson)) +
    geom_point() +
    theme_bw()
```
