...
```
Figure 2 shows that the relationship between white Elo rating and total number of moves has a varying slope and intercept when grouped by opening move. This figure also shows that as white Elo rating increases so does the total number of moves in a match. 
...
#fit <- lmer(Number_of_Moves ~ WhiteElo + (1 | Opening), Chess)
...
```{r echo= FALSE}
plot(fitted(fit), Chess$Number_of_Moves)
...
```
From this plot we can see that there is a higher density of matches that have a lower number of moves. The model appears to fits matches with lower number of moves better than those with a higher number of moves. 

```{r echo=FALSE, fig.height=2.5, fig.width=6, fig.cap= "Figure X"}
binnedplot(fitted(fit),resid(fit))
```
The binned residual plot shows that the residuals are not randomly distributed, this is indicative that the fitted model does not accurately describe the data.
...
```
Figure X shows a Residual plot and a QQ plot. The plot of the residuals vs fitted values shows that the mean of residuals is not zero. There is a non-random distribution of residuals as shown by the flat edge of the negative points. The Q-Q plot shows deviation from the standard normal line. These figures suggest that the model fit is not an accurate representation of the data.
...
limitations
inculsion of more opening moves?
nonlinear model?
